        ...
        while True:
            if random_TF:  #隨機選
                ...
                if construct_structs:  #隨機放置結構
                    struct_name = random.choice(list(construct_structs.keys()))
            else:  #手動選
                i = 1
                try:
                    if i == 1:
                    ...
                    if i == 2 and construct_structs:  # 放置結構
                        struct_name = input("%d 號出入口，放置結構的名稱 :" % number)
                except ValueError:
                    print("資料型態錯誤，請重新輸入")
                    continue
            ...
            if construct_structs:
                maze_obj.add_struct(x, y, construct_structs[struct_name], True)
            else:
